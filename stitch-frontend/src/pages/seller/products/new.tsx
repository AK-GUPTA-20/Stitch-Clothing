...
export default function PortalNewProductPage() {
  ...
  useEffect(() => {
    ...
    const foundCat = CATEGORIES.find(c => lowerName.includes(c.toLowerCase()) || 
      (c === "Outerwear & Jackets" && (lowerName.includes("jacket") || lowerName.includes("coat") || lowerName.includes("outerwear"))) ||
      (c === "Hoodies & Sweatshirts" && (lowerName.includes("hoodie") || lowerName.includes("sweatshirt"))) ||
      (c === "Jeans" && lowerName.includes("denim")) ||
      (c === "Shirts" && lowerName.includes("shirt") && !lowerName.includes("t-shirt")) ||
      (c === "T-Shirts" && (lowerName.includes("t-shirt") || lowerName.includes("tshirt") || lowerName.includes("tee")))
    );
    
    // Smart Tags
    const stopWords = new Set(["and", "the", "with", "for", "in", "on", "of", "a", "an", "men", "women", "mens", "womens"]);
    const words = form.name.toLowerCase().replace(/[^a-z0-9 ]/g, '').split(" ").filter(w => w.length > 2 && !stopWords.has(w));
    const suggestedTags = Array.from(new Set(words)).join(", ");

    setForm(prev => ({
      ...prev,
      category: foundCat && prev.category === CATEGORIES[0] ? foundCat : prev.category,
      tags: prev.tags ? prev.tags : suggestedTags
    }));
  }, [form.name]);

  const autoGenerateDescription = () => {
    if (!form.name) {
      toast.error?.("Missing Name", "Please enter a product name first.");
      return;
    }
    const mat = form.material ? ` Crafted from high-quality ${form.material.toLowerCase()},` : "";
    const occ = form.occasion ? ` perfect for ${form.occasion.toLowerCase()} wear.` : ".";
    const fitStr = form.fit ? ` It features a comfortable ${form.fit.toLowerCase()} fit` : " It features a comfortable fit";
    const desc = `Elevate your wardrobe with the new ${form.name}.${mat}${fitStr}${occ} Designed to offer both style and durability, this ${form.category?.toLowerCase() || 'piece'} is a must-have addition to your collection.`;
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      // Smart Image Sorting: Prioritize portrait images (smaller width/height ratio)
      const fileList = Array.from(files);
      const measuredFiles = await Promise.all(fileList.map(file => {
        return new Promise<{ file: File, ratio: number }>((resolve) => {
          const url = URL.createObjectURL(file);
          const img = new window.Image();
          img.onload = () => {
            URL.revokeObjectURL(url);
            resolve({ file, ratio: img.width / img.height });
          };
          img.onerror = () => {
            URL.revokeObjectURL(url);
            resolve({ file, ratio: 1 }); // fallback
          };
          img.src = url;
        });
      }));

      measuredFiles.sort((a, b) => a.ratio - b.ratio);
      const sortedFiles = measuredFiles.map(m => m.file);

      const fd = new FormData();
      sortedFiles.forEach((f) => fd.append("images", f));
      ...
      toast.success("Uploaded", `${files.length} image(s) uploaded and automatically sorted.`);
    } catch {
  ...
  const autoGenerateSizes = () => {
    ...
    const baseHex = variants[0]?.colorHex || "#000000";
    const basePrice = variants[0]?.price || form.price.toString() || "";
    
    const append = window.confirm("Do you want to append standard sizes? Clicking Cancel will clear existing variants first.");
    let newVariants = append ? [...variants] : [];
    
    defaultSizes.forEach((size, idx) => {
      if (append && newVariants.find(v => v.size === size && v.color === baseColor)) {
        return;
      }
      newVariants.push({
  ...
  const applyToAllVariants = () => {
    if (variants.length <= 1) {
      toast.error?.("Not enough variants", "Add more variants to apply to all.");
      return;
    }
    ...
    setVariants(newVariants);
    toast.success("Applied", "Price and stock copied to all variants.");
  };

  const autoGenerateSKUs = () => {
    if (!form.name) {
      toast.error?.("Missing Name", "Please enter a product name first to generate SKUs.");
      return;
    }
    const prefix = form.name.substring(0, 4).toUpperCase().replace(/[^A-Z0-9]/g, '');
    
    const newVariants = variants.map((v) => {
      const colorPart = (v.color || "DEF").substring(0, 3).toUpperCase().replace(/[^A-Z0-9]/g, '');
      const sizePart = (v.size || "NA").toUpperCase().replace(/[^A-Z0-9]/g, '');
      const sku = `${prefix}-${colorPart}-${sizePart}`.replace(/-+/g, '-');
      return { ...v, sku };
    });
    setVariants(newVariants);
  };
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 text-[10px] font-semibold text-stone-600 hover:text-stone-900 transition-colors bg-stone-100 hover:bg-stone-200 px-2 py-1 rounded-md"
                    title="Auto-generate description from name, category, material, and fit"
                  >
                    ✨ Auto-Generate
                  </button>
                </div>
            ...
            action={
              <div className="flex items-center gap-2">
                <button
                  ...
                  title="Apply first variant's price and stock to all"
                >
                  Apply to All
                </button>
                <button
                  ...
                  className="flex items-center gap-1.5 text-[11px] font-semibold text-stone-600 hover:text-stone-900 border border-stone-200 hover:border-stone-400 px-3 py-1.5 rounded-lg transition-all"
                  title="Auto-generate standard sizes"
                >
                  <RefreshCw size={11} /> Auto Sizes
                </button>
                <button
                  onClick={autoGenerateSKUs}
                  className="flex items-center gap-1.5 text-[11px] font-semibold text-stone-600 hover:text-stone-900 border border-stone-200 hover:border-stone-400 px-3 py-1.5 rounded-lg transition-all"
                  title="Auto-generate SKUs based on product name, color and size"
                >
                  <RefreshCw size={11} /> Auto SKUs
                </button>
                <button
                  onClick={addVariant}
                  className="flex items-center gap-1.5 text-[11px] font-semibold text-stone-600 hover:text-stone-900 border border-stone-200 hover:border-stone-400 px-3 py-1.5 rounded-lg transition-all"
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 mt-4">
                {uploadedImages.map((img, i) => (
                  <div key={img._id || i} className="relative aspect-[3/4] bg-stone-100 rounded-xl overflow-hidden border border-stone-200/50 group">
                    <img loading="lazy" decoding="async" src={img.url} alt={`Preview ${i + 1}`} className="w-full h-full object-cover" />
                    <button
